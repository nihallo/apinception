export const CREATE = 'CREATE';
export const UPDATE = 'UPDATE';
export const DELETE = 'DELETE';
export const FETCH_ALL = 'FETCH_ALL';
export const LIKE = 'LIKE';

export const AUTH = 'AUTH';
export const LOGOUT = 'LOGOUT';

export const API_SUBMIT = "API_SUBMIT";
export const API_UPDATE = "API_UPDATE";
export const API_DELETE = "API_DELETE";
export const API_FETCH_ALL = 'API_FETCH_ALL';

export const UPDATE_CURRENT_ID = "UPDATE_CURRENT_ID"


export const TABLE_CREATE = "TABLE_CREATE";
export const TABLE_UPDATE = "TABLE_UPDATE";
export const TABLE_DELETE = "TABLE_DELETE";
export const TABLE_FETCH_ALL = 'TABLE_FETCH_ALL';


export const API_PROCESSING_STEPS_CREATE = "API_PROCESSING_STEPS_CREATE";
export const API_PROCESSING_STEPS_UPDATE = "API_PROCESSING_STEPS_UPDATE";
export const API_PROCESSING_STEPS_DELETE = "API_PROCESSING_STEPS_DELETE";
export const API_PROCESSING_STEPS_FETCH_ALL = 'API_PROCESSING_STEPS_FETCH_ALL';

