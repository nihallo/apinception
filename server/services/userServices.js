...
export const signin = async (email, password) => {
    ...
    try {
      ...
      if (!oldUser) return errorObject("SIGN_IN_ERROR_USER_NOT_EXIST","User does not exist.");
  
      const isPasswordCorrect = await bcrypt.compare(password, oldUser.password);
  
      if (!isPasswordCorrect) return errorObject("SIGN_IN_ERROR_INVALID_CREDENTIALS","Invalid credentials."); 
      ...
      return { success:true, object:{result: oldUser, token }};
        ...
        console.log(error);
        return errorObject("SIGN_IN_ERROR_CATCH","Catch expection: Something went wrong.");
    }
  };